        ...
        <div class="h-5/6 mt-6 overflow-y-auto">
            <ul class="m-0 p-0 space-y-2">
                <c:forEach var="member" items="${approvedMembers}">
                    <li class="py-2">
                        <span class="text-gray-800">${member.regDate}</span>
                        <a href="/usr/article/transferdetail?memberId=${member.id}" class="text-blue-500 hover:underline">${member.name}</a>
                        <span class="text-gray-800">${member.department}</span>
                    </li>
                </c:forEach>
            </ul>
        </div>
        ...
        <div class="h-5/6 mt-6 overflow-y-auto">
            <ul class="p-0 space-y-2">
                <c:forEach var="suggestion" items="${showsuggestion}">
                    <li class="py-2">
                        <span class="text-gray-800">신청 사항: ${suggestion.item} 요청 결과: </span>
                        <c:choose>
        ...
        <div class="h-5/6 mt-6 overflow-y-auto">
            <ul class="p-0 space-y-2">
                <c:forEach var="file" items="${files}">
                    <li class="py-2">
                        <span class="text-gray-800">${file.id} ${file.orgName}</span>
                        <a href="/usr/article/file/${file.id}" download class="text-blue-500 hover:underline">
        ...
        <div class="h-5/6 mt-6 overflow-y-auto">
            <ul class="p-0 space-y-2">
                <c:forEach var="vacation" items="${showVacation}">
                    <li class="py-2">
                        <span class="text-gray-800">${vacation.date} ${vacation.vacationType1}${vacation.vacationType2}</span>
                        <c:choose>
